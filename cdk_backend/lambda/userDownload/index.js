...
export const handler = async (event) => {
  ...
  if (!key) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Missing file key' }) };
  }
...
//   }
  ...
  try {
    ...
    if (err.name === 'NotFound') {
      ...
      return { statusCode: 200, body: JSON.stringify({ notFound: true }) };
    }
    console.log(`Error generating presigned download url for ${key}.`)
    return { statusCode: 500, body: JSON.stringify({ error: 'Error during S3 download process.' })}
  }
};
